...
const tl1 = gsap.timeline({
    scrollTrigger: {
        ...
        pin: true,            
        scrub: 1.5, // Reducido un poco para mayor respuesta reactiva suave
        anticipatePin: 1
    }
});
...
tl1.to("#text-box-1", { opacity: 1, y: 0, duration: 1.2, ease: "power3.out" })
   ...
   .to("#animal-1", { y: -40, scale: 0.95, duration: 1.2, ease: "power1.inOut" });
...
tl2.to("#animal-2", { opacity: 1, scale: 1.1, rotation: -3, duration: 1.5, ease: "expo.out" })
   .to("#text-box-2", { opacity: 1, y: 0, duration: 1.2, ease: "power3.out" }, "-=0.9")
   .to("#animal-2", { y: -30, scale: 1.02, duration: 1.2, ease: "power1.inOut" });

// --- SECCIÓN 3: ECOSISTEMAS (ÁGUILA) ---
...
tl3.to("#title-3", { opacity: 1, scale: 1, y: 0, duration: 1.2, ease: "power4.out" })
   ...
   .to("#animal-3", { bottom: "40px", opacity: 0.5, scale: 1.2, duration: 1.5, ease: "power2.out" }, "-=1.2");